...
Official released binaries and the `ghcr.io/aifity/omnigit-mcp` image ship
...
```bash
omnigit-mcp stdio --oauth-client-id <YOUR_CLIENT_ID>
```
...
```bash
go build -ldflags "\
  -X github.com/aifity/omnigit-mcp/internal/buildinfo.OAuthClientID=<CLIENT_ID> \
  -X github.com/aifity/omnigit-mcp/internal/buildinfo.OAuthClientSecret=<CLIENT_SECRET>" \
  ./cmd/omnigit-mcp
```
