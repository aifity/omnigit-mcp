...
`get_review_comments` now returns a `CommentID` field extracted from the URL fragment (e.g., `#discussion_r2837328047`) alongside the GraphQL `NodeID` and `DatabaseID` for every comment node. This `CommentID` is the numeric ID that should be used when calling `add_reply_to_pull_request_comment` or `pull_request_comment_write`. Previously, the model had to manually extract this ID from the URL or decode the base64 GraphQL node ID. The `commentId` / `comment_id` parameter descriptions in those tools now explicitly say to use the `CommentID` field.
...
The remote GitHub MCP Server is hosted by GitHub and provides the easiest method for getting up and running. If your MCP host does not support remote MCP servers, don't worry! You can use the [local version of the GitHub MCP Server](https://github.com/aifity/omnigit-mcp?tab=readme-ov-file#local-omnigit-mcp) instead.

### Prerequisites

1. A compatible MCP host with remote server support (VS Code 1.101+, Claude Desktop, Cursor, Windsurf, etc.)
2. Any applicable [policies enabled](https://github.com/aifity/omnigit-mcp/blob/main/docs/policies-and-governance.md)
...
[![Install with Docker in VS Code](https://img.shields.io/badge/VS_Code-Install_Server-0098FF?style=flat-square&logo=visualstudiocode&logoColor=white)](https://insiders.vscode.dev/redirect/mcp/install?name=github&inputs=%5B%7B%22id%22%3A%22github_token%22%2C%22type%22%3A%22promptString%22%2C%22description%22%3A%22GitHub%20Personal%20Access%20Token%22%2C%22password%22%3Atrue%7D%5D&config=%7B%22command%22%3A%22docker%22%2C%22args%22%3A%5B%22run%22%2C%22-i%22%2C%22--rm%22%2C%22-e%22%2C%22GITHUB_PERSONAL_ACCESS_TOKEN%22%2C%22ghcr.io%2Faifity%2Fomnigit-mcp%22%5D%2C%22env%22%3A%7B%22GITHUB_PERSONAL_ACCESS_TOKEN%22%3A%22%24%7Binput%3Agithub_token%7D%22%7D%7D) [![Install with Docker in VS Code Insiders](https://img.shields.io/badge/VS_Code_Insiders-Install_Server-24bfa5?style=flat-square&logo=visualstudiocode&logoColor=white)](https://insiders.vscode.dev/redirect/mcp/install?name=github&inputs=%5B%7B%22id%22%3A%22github_token%22%2C%22type%22%3A%22promptString%22%2C%22description%22%3A%22GitHub%20Personal%20Access%20Token%22%2C%22password%22%3Atrue%7D%5D&config=%7B%22command%22%3A%22docker%22%2C%22args%22%3A%5B%22run%22%2C%22-i%22%2C%22--rm%22%2C%22-e%22%2C%22GITHUB_PERSONAL_ACCESS_TOKEN%22%2C%22ghcr.io%2Faifity%2Fomnigit-mcp%22%5D%2C%22env%22%3A%7B%22GITHUB_PERSONAL_ACCESS_TOKEN%22%3A%22%24%7Binput%3Agithub_token%7D%22%7D%7D&quality=insiders)
...
- **search_code** - Search code
  ...
  - `page`: Page number for pagination (min 1) (number, optional)
  - `perPage`: Results per page for pagination (min 1, max 100) (number, optional)
  - `query`: Search query using GitHub's powerful code search syntax. Examples: 'content:Skill language:Java org:github', 'NOT is:archived language:Python OR language:go', 'repo:aifity/omnigit-mcp'. Supports exact matching, language filters, path filters, and more. (string, required)
...
- issue bodies (create / update)
- pull request bodies (create / update)
- local git commit messages (when using the `local_git` toolset)

By default, the server filters:

- `Co-Authored-By:` trailers
- a generic PR footer like `---\nPull Request opened by [Tool](url) with guidance from ...`
